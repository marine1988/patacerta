...
const DestaqueTabLazy = lazy(() =>
  import('./DestaqueTab').then((m) => ({ default: m.DestaqueTab })),
)
...
export default function DashboardPage() {
  ...
  // Inclui `status` para gating do tab "Destaque" (apenas VERIFIED).
  const { data: breederProbe } = useQuery<{ id: number; status: string } | null>({
  ...
  const isVerifiedBreeder = breederProbe?.status === 'VERIFIED'
      ...
      ? [
          {
            ...
            content: withFallback(<DestaqueTabLazy />),
          },
        ]
      : []),
    ...(showServicesTab
